...
local function lock_group_etehad(msg, data, target)
  ...
  local group_super_lock = data[tostring(target)]['settings']['super']
  if group_etehad_lock == 'yes' then
    return '<b>SUPER setting is already locked</b>'
  else
    data[tostring(target)]['settings']['super'] = 'yes'
    save_data(_config.moderation.data, data)
    return '<b>SUPER setting has been locked</b>'
  end
end
...
local function unlock_group_etehad(msg, data, target)
  ...
  local group_etehad_lock = data[tostring(target)]['settings']['super']
  if group_etehad_lock == 'no' then
    return '<b>SUPER setting is not locked</b>'
  else
    data[tostring(target)]['settings']['super'] = 'no'
    save_data(_config.moderation.data, data)
    return '<b>SUPER setting has been unlocked<b>'
  end
end
...
function show_supergroup_settingsmod(msg, target)
	...
	if data[tostring(target)]['settings'] then
		if not data[tostring(target)]['settings']['super'] then
			data[tostring(target)]['settings']['super'] = 'no'
		end
	end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "<b>@"..result.from.username.." [ "..result.from.peer_id.." ] added as owner</b>"
			else
				text = "<b>[ "..result.from.peer_id.." ] added as owner</b>"
			end
			send_large_msg(channel_id, text)
		end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == "owner" then
			...
			return "<b>SuperGroup owner is ["..group_owner..']‌</b>'
		end
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "<i>SuperGroup ID for</i> " ..string.gsub(msg.to.print_name, "_", " ").. ":\n"..msg.to.id
			end
		end
